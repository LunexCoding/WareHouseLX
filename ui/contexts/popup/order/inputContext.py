...
class InputOrderContext(Context):
    def __init__(self, window, data):
        ...
        self._entries = {}
        self._namesMainFields = {field: fieldData["text"] for field, fieldData in Order.getInputFields().items() if field in Order.getMainInputFields()}
        window.title(self._name)
        window.geometry(f"{Constants.WINDOW_WIDTH}x{60 * (len(Order.getInputFields()) + 3)}")
        ...
        for field in Order.getInputFields():
            CTkLabel(
                self.frame,
                text=Order.getInputFields()[field]["text"],
                font=Constants.FONT
            ).pack(pady=5)
            entry = CTkEntry(
                self.frame,
                font=Constants.FONT,
                width=Order.getInputFields()[field]["size"]
            )
            entry.pack(pady=5)
